...
function readteam(setting){
	setting.dir="up" //initial state of panel (up="contracted")
	if (setting.stateconfig.persiststate && readteam.getCookie(setting.ids[0])=="down"){
		setting.dir="down"
	}
	...
	if (window.addEventListener) //if non IE browsers, initialize panel window.onload
		readteam.addEvent(window, function(e){thispanel.initpanel(setting)}, "load")
	else //else if IE, add 100 millisec after window.onload before initializing
		readteam.addEvent(window, function(e){setTimeout(function(){thispanel.initpanel(setting)}, 100)}, "load")
	readteam.addEvent(window, function(e){thispanel.uninit(setting)}, "unload")
}

readteam.events_array=[] //object array to contain events created by script

readteam.addEvent=function(target, functionref, tasktype){
	var evtmodel=target.addEventListener? "w3c" : "ie"
	var evtaction=evtmodel=="w3c"? "addEventListener" : "attachEvent"
	var i=this.events_array.push({ //store event info in readteam.events_array[] and return current event's index within array
		target: target,
...
readteam.removeEvent=function(target, functionref, tasktype){
...
readteam.getCookie=function(Name){ 
...
readteam.setCookie=function(name, value){
	document.cookie = name+"=" + value + ";path=/"
}

readteam.addpointer=function(target, className, imagesrc){
...
togglepanel:function(dir){ //public function that toggles the panel's state. Optional dir parameter ("up" or "down") to explicitly set state.
	...
	readteam.setCookie(setting.ids[0], setting.dir)
},
...
initpanel:function(){
	...
	var thispanel=this
	readteam.addEvent(tdiv, function(e){ //assign click behavior when toggle DIV tab is clicked on
	...
	if (setting.pointerimage.enabled){
		...
		setting.arrow=readteam.addpointer(tdiv.getElementsByTagName("span")[0], "pointerimage", setting.pointerimage.src[setting.dir=="down"? 1:0])
	}
	if (setting.closepanelonclick.enabled){ //assign click behavior when panel content is clicked on (links within panel or elements with class "closepanel"
			readteam.addEvent(pcontent, function(e){
...
} //end of readteam object


//initialize instance of DD Drop Down Panel:

var defaultpanel=new readteam({
	ids: ["tls", "tlscontent", "tlstab"], // id of main panel DIV, content DIV, and tab DIV
	...
	pointerimage: {enabled: false, src: ["home16.gif", "arrow-up.gif"]},
